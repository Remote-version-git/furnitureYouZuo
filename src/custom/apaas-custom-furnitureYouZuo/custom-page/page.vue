...
<template>
  <div class="apaas-custom-page-furnitureYouZuo custom-page">
    ...
    <div class="page-header">
      ...
      <div class="operation">
        <x-svg-icon :name="'refresh-icon'" class="pointer" style="cursor: pointer;" @click.native="handleRefresh()">
        </x-svg-icon>
      </div>
    </div>
    <div class="page-search-panel">
      <el-form class="search-list">
        <template v-for="searchConfig in searchListConfig">
          <component :is="searchConfig.component" :key="searchConfig.searchKey" :label="searchConfig.label"
            :extraConfig="searchConfig.extraConfig" :searchKey="searchConfig.searchKey"
            :searchValue="searchPanelValue[searchConfig.searchKey]"
            @change="handleSearchValueChange(searchConfig.searchKey, $event)"></component>
        </template>
      </el-form>
    ...
    <div class="page-data-list">
      <x-vxe-table ref="vxeTable" :border="true" :seqWidth="40" :colConfigs="tableConfig.colConfigs"
        :tableData="tableData" :rowDraggable="tableConfig.rowDraggable" :seqType="tableConfig.seqType"
        :pagination="pagination" :layout="tableConfig.layout" :seqConfig="tableConfig.seqConfig"
        :autoSize="tableConfig.autoSize" :pageConfig="tableConfig.pageConfig" class="block-table"
        @table-row-click="rowClickEvent" @size-change="pageSizeChange" @current-page-change="currentChange"
        @select-data-change="selectDataChange">
        <template v-for="(colConfig, index) in tableConfig.colConfigs" v-slot:[colConfig.customHeadSlot]="{ column }">
        ...
        <template v-for="(colConfig, index) in tableConfig.colConfigs"
          ...
          <div v-if="colConfig.customSlot === 'options'" :key="index + '' + rowIndex">
            <a href="javascript:void(0)" class="el-link el-link--primary" @click.stop="handleRowDetail(row)">详情</a>
            <span style="width: 4px; display: inline-block"></span>
            <a href="javascript:void(0)" class="el-link el-link--primary" @click.stop="handleRowEdit(row)">编辑</a>
          </div>
          <div v-if="colConfig.customSlot === 'status'" :key="index + '' + rowIndex">
            <span v-if="row[colConfig.prop]"
              style="background: #67C23A; padding: 4px 8px; color: white; border-radius: 20px;">生效</span>
            <span v-if="!row[colConfig.prop]"
              style="background: #F56C6C; padding: 4px 8px; color: white; border-radius: 20px;">失效</span>
          </div>
        </template>
        <template slot="empty">
